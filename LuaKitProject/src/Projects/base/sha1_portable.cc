...
std::string SecureHashAlgorithm::ToHex()
{
    const char*buffer = (char *)Digest();
    size_t len = base::SecureHashAlgorithm::kDigestSizeBytes;
    ...
    for(int i = 0; i< (int)len;i++)
    {
        ...
        snprintf(tmp,sizeof(tmp)-1,"%02x",(unsigned char)ptr[i]);
