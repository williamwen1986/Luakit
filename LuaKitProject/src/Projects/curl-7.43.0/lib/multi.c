                                   ...
                                   CURL *curl_handle)
{
  ...
  Curl_expire(data, 0); /* Note: This must do after Curl_done, because 
                         something protocol list imap/smtp will send command 
                         when call Curl_done and insert 'data' into splay_tree
                         again, but the data's 'timeoutlist' would be delete
                         immediately.
                         */
  
  /* destroy the timeout list that is held in the easy handle, do this *after*
   Curl_done() as that may actually call Curl_expire that uses this */
